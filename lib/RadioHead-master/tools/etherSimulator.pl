...
my $filter = POE::Filter::Block->new(LengthCodec = > [\ & encoder,
\&decoder ] );

# Message types
# See RH_TcpProtocol.h
my $RH_TCP_MESSAGE_TYPE_NOP = 0;
#
Not used
my $RH_TCP_MESSAGE_TYPE_THISADDRESS = 1;
#
Specifies the
thisAddress of
the connected
...
# given the message length and the bits per second
            ...
            if ($elapsed > length($$value{'packet'}) * 8 / $bps) {
                ...
                delete $$value{'packet'}; # Delivered, forget
                it
            }
        }
        }
...
my ($length, $type, $packet
) = unpack('NCa*', $client_input);
...
unless willDeliverFromTo($clients{$client} { 'thisaddress' }
